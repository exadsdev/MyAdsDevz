...
const emptyVideo = {
  ...
  author: "ทีมวิดีโอ",
  duration: "PT5M",
  thumbnail: "", // ✅ เก็บเป็น path เช่น /uploads/videos/xxx.webp หรือ /videos/thumb.jpg
...
/** ✅ แปลงค่า thumbnail เป็น URL สำหรับ preview (เหมือนหน้าโพสต์) */
...
/** ✅ ใช้ API ร่วมกัน: /api/admin/uploads */
...
// รองรับ: 00:00 ข้อความ / [00:00] ข้อความ / 00:00:10 ข้อความ
...
function parseFaqRaw(raw = "") {
  ...
  // โหมด B: Question? แล้วตามด้วย answer lines
  let q = "";
  let a = [];
...
export default function AdminVideos() {
  ...
  // ✅ Thumbnail upload UI แบบเดียวกับหน้าโพสต์
  ...
  // ✅ อัปโหลด thumbnail (ใช้ /api/admin/uploads เหมือนหน้าโพสต์)
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((v) => {
      ...
      return (v.title || "").toLowerCase().includes(q) || (v.slug || "").toLowerCase().includes(q) || kw.includes(q);
    });
  }, [items, filter]);

  const ytId = extractYoutubeId(form.youtube);

  // ✅ thumbSrc ใช้แบบเดียวกับเดิม แต่ถ้า thumbnail ว่างจะ fallback ไป YouTube
      ...
      <div className="row g-4">
        <div className="col-12 col-lg-6">
          <form className="card p-3 border-0 shadow-sm" onSubmit={onSubmit}>
            ...
            {/* ✅ Thumbnail แบบเดียวกับหน้าโพสต์ */}
            ...
            {thumbTab === "url" ? (
              ...
              <div className="mb-3">
                <input type="file" accept="image/*" className="form-control" onChange={(e) => setThumbFile(e.target.files?.[0] || null)} />
                <button
                  type="button"
                ...
                <div className="form-text">อัปโหลดจะเรียก <code>/api/admin/uploads</code> และบันทึก path ลงช่อง Thumbnail อัตโนมัติ</div>
              </div>
            )}
            ...
            {(form.thumbnail || ytId) && (
              <div className="mb-3 text-center">
                ...
                <div className="small text-muted mt-1">
                  URL จริง: <code>{thumbSrc}</code>
                </div>
              </div>
            )}

            <label className="form-label">Tags (คั่นด้วย ,)</label>
            <input className="form-control mb-3" value={form.tags.join(", ")} onChange={(e) => setForm({ ...form, tags: normalizeCsv(e.target.value) })} />

            <label className="form-label">Keywords (คั่นด้วย ,)</label>
            <input className="form-control mb-3" value={form.keywords.join(", ")} onChange={(e) => setForm({ ...form, keywords: normalizeCsv(e.target.value) })} />

            <label className="form-label">เนื้อหา/ไฮไลท์ (HTML)</label>
            <textarea className="form-control mb-3" rows={5} value={form.contentHtml} onChange={(e) => setForm({ ...form, contentHtml: e.target.value })} />
            ...
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex gap-2 align-items-center flex-wrap">
                <div className="form-check form-switch mb-0">
                  <input className="form-check-input" type="checkbox" id="autoTranscript" checked={autoTranscript} onChange={(e) => setAutoTranscript(e.target.checked)} />
                ...
                <div className="form-check form-switch mb-0">
                  <input className="form-check-input" type="checkbox" id="autoChapters" checked={autoChapters} onChange={(e) => setAutoChapters(e.target.checked)} />
